...
- **Campaign** (`linkedin/models.py`) — `name` (unique), `users` (M2M to User for membership), `product_docs`, `campaign_objective`, `booking_link`, `is_freemium` (bool, deprecated/unused), `action_fraction` (float), `seed_public_ids` (JSONField).
...
Freemium campaign support is deprecated/disabled. (Historically, `ConnectStrategy` handled differences based on `campaign.is_freemium`).
...
- Freemium campaigns: [DEPRECATED/UNUSED] `find_freemium_candidate()` from `pipeline/freemium_pool.py` with just-in-time Deal creation.
...
- **`freemium_pool.py`** [DEPRECATED/UNUSED] — `find_freemium_candidate()`: queries `Lead` for embedded leads without a Deal in the campaign.
...
- **`deals.py`** — Deal/state operations: `set_profile_state()`, `get_qualified_profiles()`, `get_ready_to_connect_profiles()`, `get_profile_dict_for_public_id()`, `increment_connect_attempts()`, `create_disqualified_deal()`, `create_freemium_deal()` (deprecated/unused).
...
- [DEPRECATED/UNUSED] Standalone qualifier for freemium campaigns. Wraps a pre-trained sklearn-compatible model as a black-box scorer. No inner BayesianQualifier.
...
- `fetch_kit()` [DEPRECATED/UNUSED] — Downloads freemium campaign kit from HuggingFace (`eracle/campaign-kit`), loads `config.json` + `model.joblib`. Cached after first attempt.

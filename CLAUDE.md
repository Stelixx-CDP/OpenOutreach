...
- **LLM Gateway**: Recommended: 9router as `openai_compatible` provider. Set `LLM_PROVIDER=openai_compatible`, `LLM_API_BASE=https://your-9router/v1`, `LLM_API_KEY=...` in SiteConfig (Django Admin). 9router handles auto-fallback between Gemini/Ollama/CF Worker. Direct provider connections (OpenAI, Anthropic, etc.) still supported.
